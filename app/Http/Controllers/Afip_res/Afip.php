 ...
 * @version 0.5
 **/
namespace App\Http\Controllers\Afip_res;
...
class Afip{
	...
	function __construct($options)
	{
		...
		if (!isset($options['res_folder'])) {
			...
			$this->RES_FOLDER = app_path().'/Http/Controllers/Afip_res/';
		} else {
			$this->RES_FOLDER = $options['res_folder'];
		}

		if (!isset($options['ta_folder'])) {
			$this->TA_FOLDER = app_path().'/Http/Controllers/Afip_res/';
		} else {
		...
		$this->WSAA_WSDL 	= app_path().'/Http/Controllers/Afip_res/'.'wsaa.wsdl';
	...
	public function __get($property)
	{
		if (in_array($property, $this->implemented_ws)) {
			if (isset($this->{$property})) {
				...
				$file = app_path (). '/Http/Controllers/Afip_res/ClassT/'.$property.'.php';
				if (!file_exists($file))
...
class AfipWebService
{
	...
	function __construct($afip)
	{
		...
		if ($this->afip->options['production'] === TRUE) {
			$this->WSDL = app_path (). '/Http/Controllers/Afip_res/'.$this->WSDL;
		} else {
			$this->WSDL = app_path (). '/Http/Controllers/Afip_res/'.$this->WSDL_TEST;
