...
<section class="content-header">
      ...
      <ol class="breadcrumb">
        <li><a href="#"><i class="fas fa-file-invoice-dollar"></i> Presupuestos</a></li>
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
                <br>
              <h3 class="box-title">Presupuestos Cargados</h3>
            </div>
            ...
            <button type="button" class="btn btn-default" data-toggle="modal" data-target="#modal-default">
              ...
              <br>
              <br>
              <table id="tablePreVentas" class="table table-bordered table-striped">
                <thead>
                <tr>
                  <th>id</th>
                  <th>Cliente</th>
                ...
                <tbody>
                  @foreach($Preventas as $preventa)
                    <tr>
                       <th>{{$preventa->id}}</th>
                       <th>{{$preventa->cliente->nombre}}</th>
                       <th>{{$preventa->fecha}}</th>
                       <th>{{$preventa->total}}</th>
                       <th>{{$preventa->subtotal}}</th>
                       @if($preventa->estado == 'Pendiente')
                       <th><span class="badge badge-pill badge-red bg-red">Pendiente</span></th>
                       <th>
                         <form id="venderPreventa" class="" action="venderPreventa" method="POST" target="_blank">
                            {{ csrf_field() }}
                            <input type="hidden" name="id" value="{{$preventa->id}}">
                             <a href=""><button class="btn btn-success" type="submit">Vender</button></a>
                         </form>
                        </th>
                        <th>
                           <form id="facturarPreventa" class="" action="facturarPreventa" method="POST" target="_blank">
                              {{ csrf_field() }}
                              <input type="hidden" name="id" value="{{$preventa->id}}">
                       ...
                       @else
                        <th><span class="badge badge-pill badge-blue bg-green">{{$preventa->estado}}</span></th>
                        <th>
                          <form id="venderPreventa" class="" action="venderPreventa" method="POST" target="_blank">
                             {{ csrf_field() }}
                             <input type="hidden" name="id" value="{{$preventa->id}}">
                         ...
                         <th>
                            <form id="facturarPreventa" class="" action="facturarPreventa" method="POST" target="_blank">
                               {{ csrf_field() }}
                               <input type="hidden" name="id" value="{{$preventa->id}}" disabled>
                       ...
                       <th>
                         <form id="getFactura" class="" action="getPreventa" method="POST" target="_blank">
                            {{ csrf_field() }}
                            <input type="hidden" name="id" value="{{$preventa->id}}">
    ...
    <script>
        ...
        var tablePreVenta = $('#tablePreVenta').DataTable({
               'paging'      : true,
               'lengthChange': true,
               'searching'   : true,
               'ordering'    : true,
        ...
        $(document).ready(function(){
            table();
            $('#SendPreventa').click(function(){
              var table=$('#tablePreVenta').DataTable();
                ...
                $('#items').val(items);
                $('#neto').val(neto);
                document.getElementById("formPreVenta").submit();
          ...
          $("#cliente").easyAutocomplete(options);
            ...
            $('#saveItem').click(function(){
                 ...
                 var results = $('#tablePreVenta').DataTable();
    ...
    $('#tableProductos').DataTable( {
        ...
        select: {
            ...
            $("#tablePreVenta").on('click', '.btnDelete', function () {
                var results = $('#tablePreVenta').DataTable();
            ...
            $("#tablePreVenta").on('change', '.cantidadProducto', function () {
                var results = $('#tablePreVenta').DataTable();
        ...
        function CalcularTotal(){
            ...
            $('#TotalPreVenta').val(total.toFixed(2));
        ...
        <div class="modal-dialog bs-example-modal-lg modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">ABM Presupuestos</h4>
                </div>
                ...
                <div class="modal-body" style="height: 450px;overflow-y: auto;">
                        ...
                        <div class="tab-content">
                            ...
                            <div class="active tab-pane" id="Factura">
                              <form id="formPreVenta" class="" action="addPreventa" method="POST" target="_blank">
                            ...
                            <div class="col-lg-12">
                                ...
                                <table id="tablePreVenta" class="table table-bordered table-striped">
                ...
                <div class="modal-footer">
                  ...
                  <div class="col-lg-4 text-center">
                    ...
                    <input type="hidden" value="0.00" name="TotalPreVenta" id="TotalPreVenta">
                    ...
                    <button type="button" class="btn btn-primary" id="SendPreventa">Crear Preventa</button>
                </div>
